...
async def get_widget_config(widget_id: str):
    widget = widget_svc.get_widget(widget_id)
    if not widget:
        raise HTTPException(status_code=404, detail="Widget not found")
    embed = widget_svc.generate_embed_snippet(widget_id)
    return JSONResponse(
...
async def list_widgets(tenant: dict = Depends(require_tenant)):
    widgets = widget_svc.get_for_tenant(tenant["tenant_id"])
...
async def create_widget(body: dict, tenant: dict = Depends(require_tenant)):
    widget = widget_svc.create_widget(tenant["tenant_id"], body)
    embed = widget_svc.generate_embed_snippet(widget["widget_id"])
...
async def update_widget(widget_id: str, body: dict, tenant: dict = Depends(require_tenant)):
    widget = widget_svc.update_widget(widget_id, tenant["tenant_id"], body)
    if not widget:
...
async def delete_widget(widget_id: str, tenant: dict = Depends(require_tenant)):
    ok = widget_svc.delete_widget(widget_id, tenant["tenant_id"])
    if not ok:
        raise HTTPException(status_code=404, detail="Widget not found or access denied")
